...
public class MenuPanel extends JPanel implements Updateable{
	...
	public MenuPanel(Observer o)
	{
		...
		bCreateDefaultMap.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if(observer.mapIsInitialized == false) {
					observer.makeDefaultMap();
					observer.mapIsInitialized = true;
					initializeMenu();
				}
			}
		});
		...
		bInitializeMap.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(observer.mapIsInitialized == false) {
					String fileName1 = tEdge.getText();
					String fileName2 = tNode.getText();
					try {
						observer.initializeGameEnvironment(fileName1, fileName2);
					} catch (Exception e1) {
						e1.printStackTrace();
					}
					observer.mapIsInitialized = true;
